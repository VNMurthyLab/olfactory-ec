...
def W_heatmap(ax, W, config_path): 
    ...
    ax.set_title("Optimized W (fly larva)")
    ax.set_yticks([0, 20])
    ax.set_yticklabels(["1", "21"])
...
fig.savefig(f"W_fly_larva_dual_heatmap_clip_{CLIP:.0e}.png", dpi=600)
fig.savefig(f"W_fly_larva_dual_heatmap_clip_{CLIP:.0e}.pdf", dpi=600)
fig.savefig(f"W_fly_larva_dual_heatmap_clip_{CLIP:.0e}.svg")
